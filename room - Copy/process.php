    	 ...
    	 case ('send'):
    		 ...
    		 if (!preg_match($blankexp, $message)) {
    			 ...
    			 if (preg_match($reg_exUrl, $message, $url)) {
           			$message = preg_replace($reg_exUrl, '[URL links are not allowed]', $message);
    			 } 
    			 $message = preg_replace($patterns, $replacements, $message);
            	
            	 fwrite(fopen($file, 'a'), '<span id="time-span">' . date("H:i:s",$time_stamp) . '</span><span id="name-span">'. $nickname . "</span>" . $message = str_replace("\n", " ", $message) . "\n"); 
    		 }
